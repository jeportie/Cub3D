...
/*   Updated: 2025/02/21 17:44:15 by jeportie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_map(t_data *data)
{
	...
	restore_map(data);
	print_map(data);
	data->map = ft_strjoin_array(data->parse.map.layout, data->parse.map.height - 1);
	return (0);
}
...
int hole_in_the_map(t_data *data, int i, int j)
{
	...
	if (i == 0 || i == data->parse.map.height - 1 || j == 0 || j == (int)ft_strlen(data->parse.map.layout[i]) - 1)
		return (1);
...
int	floodfill_algo(t_data *data, int i, int j)
{
	...
	if (!data->parse.map.layout[i][j])
		return(0);
	if (data->parse.map.layout[i][j] == '1' || data->parse.map.layout[i][j] == 'P')
		return(0);
	else

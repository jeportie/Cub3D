...
/*   Updated: 2025/02/24 14:45:56 by anastruc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_first_last_row(t_data *data)
{
	...
	j = 0;
	while (j < ((int)ft_strlen (data->parse.map.layout[data->parse.map.height - 1])))
	{
...
int	ft_check_flag(int flag, t_data *data)
{
	...
	else if (flag > 1)
	{
		printf("\033[31mError\nToo many player's positions. Extra at[%d][%d]"
			"Extra = %d\n\033[0m",
			data->parse.map.player_i, data->parse.map.player_j, flag - 1);
		return (ft_clean_data_and_exit(data));
	}
	else
		return (0);
